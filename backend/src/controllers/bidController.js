...
exports.createBid = async (req, res) => {
  try {
    const { loadId, amount, message, proposedPickupDate, proposedDeliveryDate } = req.body;
...
async function checkTruckerEligibility(trucker, load) {
  ...
  if (criteria.maxAccidentHistory !== undefined) {
    if (trucker.accidentHistory.hasAccidents) {
      const accidentCount = trucker.accidentHistory.details ? trucker.accidentHistory.details.length : 0;
      if (accidentCount > criteria.maxAccidentHistory) {
        result.isEligible = false;
        result.reasons.push(`You have ${accidentCount} accidents in your history, but the maximum allowed is ${criteria.maxAccidentHistory}`);
      }
    }
  }
  ...
  if (criteria.maxTheftComplaints !== undefined) {
    if (trucker.theftComplaints.hasComplaints) {
      const complaintCount = trucker.theftComplaints.details ? trucker.theftComplaints.details.length : 0;
      if (complaintCount > criteria.maxTheftComplaints) {
        result.isEligible = false;
        result.reasons.push(`You have ${complaintCount} theft complaints in your history, but the maximum allowed is ${criteria.maxTheftComplaints}`);
      }
    }
  }
  ...
  if (criteria.maxTruckAge !== undefined) {
    ...
    if (truckAge > criteria.maxTruckAge) {
      result.isEligible = false;
      result.reasons.push(`Your truck is ${truckAge} years old, but the maximum age allowed is ${criteria.maxTruckAge} years`);
    }
  }
  ...
  if (criteria.minExperienceYears !== undefined) {
    ...
    const experienceYears = Math.floor((today - issueDate) / (365.25 * 24 * 60 * 60 * 1000));
    
    if (experienceYears < criteria.minExperienceYears) {
      result.isEligible = false;
      result.reasons.push(`You have ${experienceYears} years of driving experience, but the minimum required is ${criteria.minExperienceYears} years`);
    }
  }
...
exports.getBid = async (req, res) => {
  try {
    ...
    const isBidder = trucker && bid.trucker.toString() === trucker._id.toString();
